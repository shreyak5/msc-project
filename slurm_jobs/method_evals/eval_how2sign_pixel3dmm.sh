...
#SBATCH --job-name=eval_how2sign_pixel3dmm
#SBATCH --output=/home/u6ga/sk3925.u6ga/sk3925_project/msc-project/slurm_jobs/output/eval_how2sign_pixel3dmm_%j.out
#SBATCH --nodes=4
#SBATCH --ntasks=16
...
# the full test set (2,343 clips - the largest of the three datasets) needs many more
# shards than eval_how2sign.sh's single 4-GPU node to fit under this cluster's 24h
# wall-time cap - topology copied from slurm_jobs/prewarm_mica_cache.sh (proven in
# production: see its slurm_jobs/output/prewarm_mica_cache_5656216.out, 16 shards
# completed cleanly, no CUDA/device errors). --gres=gpu:4 must be passed to srun itself,
# not just #SBATCH - see slurm_jobs/pretrain.sh's comment on a real prior failure from
# omitting it.
#
...
OUTPUT_DIR=evaluation/output_dataset_pixel3dmm/how2sign
NUM_SHARDS=16
