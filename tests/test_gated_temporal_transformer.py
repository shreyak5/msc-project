...
from model.config import GatedTTConfig  # noqa: E402
from model.temporal import GatedTemporalTransformer, _gather_local_windows  # noqa: E402
...
def _key_col_index(query_frame, key_frame, key_component, radius, num_components):
    """Column index into distance_bias's/visibility_gate's (w*num_components) key
...
def test_alibi_slopes_match_config():
    cfg = GatedTTConfig()
    tt = GatedTemporalTransformer(cfg)
...
def test_padded_frames_do_not_affect_valid_frame_outputs():
    ...
    visibility_b[0, 4:] = 999.0  # clamped to 1.0 by _compute_visibility_gate anyway
...
def test_window_size_is_configurable():
    tt = GatedTemporalTransformer(GatedTTConfig(window_size=3))
    num_components = 4
    num_frames = 5
    radius = 1
    ...
    bias = tt._compute_distance_bias(frame_indices, valid_mask, num_components)
    gate = tt._compute_visibility_gate(visibility, valid_mask, num_components)
    assert bias.shape == (1, num_frames, tt.config.num_heads, num_components, seq_len_kv)
    assert gate.shape == (1, num_frames, num_components, seq_len_kv)
...
def test_local_attention_is_actually_local():
    ...
    torch.manual_seed(0)
    tt = GatedTemporalTransformer(GatedTTConfig(window_size=5))
    with torch.no_grad():
        tt.output_proj.weight.add_(torch.randn_like(tt.output_proj.weight) * 0.02)
    ...
    visibility_b = visibility.clone()
    visibility_b[0, perturbed_frame] = 0.01
...
def test_visibility_gate_matches_formula():
    cfg = GatedTTConfig(gamma=2.0)
    tt = GatedTemporalTransformer(cfg)
    num_components = 4
    num_frames = 4
    radius = tt.config.window_size // 2
    visibility = torch.tensor([[0.9, 0.4, 0.5, 0.5]])
    valid_mask = torch.ones(1, num_frames, dtype=torch.bool)

    gate = tt._compute_visibility_gate(visibility, valid_mask, num_components)
    col = _key_col_index(query_frame=0, key_frame=1, key_component=0, radius=radius, num_components=num_components)
    assert abs(gate[0, 0, 0, col].item() - 0.4**2.0) < 1e-5


def test_visibility_gate_masks_padded_keys():
    tt = GatedTemporalTransformer()
    num_components = 4
    num_frames = 4
    radius = tt.config.window_size // 2
    ...
    gate = tt._compute_visibility_gate(visibility, valid_mask, num_components)
    col = _key_col_index(query_frame=0, key_frame=2, key_component=0, radius=radius, num_components=num_components)
    assert (gate[0, 0, :, col] == 0.0).all()


def test_gamma_zero_approaches_uniform_gate():
    tt = GatedTemporalTransformer(GatedTTConfig(gamma=0.0))
    num_components = 4
    num_frames = 4
    radius = tt.config.window_size // 2
    visibility = torch.tensor([[0.1, 0.9, 0.3, 0.5]])
    valid_mask = torch.ones(1, num_frames, dtype=torch.bool)

    gate = tt._compute_visibility_gate(visibility, valid_mask, num_components)
    for key_frame in range(num_frames):
        col = _key_col_index(
            query_frame=0, key_frame=key_frame, key_component=0, radius=radius, num_components=num_components
        )
        assert abs(gate[0, 0, 0, col].item() - 1.0) < 1e-6


def test_gating_redistributes_attention_toward_higher_visibility():
    # Direct, mechanism-level test of the gate+renormalize math (the same formula
    # GatedTTBlock.forward applies to real softmax weights): with a uniform
    # starting attention distribution, multiplying by a gate that singles out one
    # key as much-less-visible and renormalizing should shrink that key's own
    # weight below its ungated share and shift the freed mass to its neighbors -
    # confirming the "weight migrates away from low-visibility positions" behavior.
    raw_weights = torch.full((5,), 0.2)  # uniform over a window of 5 keys
    gate = torch.tensor([1.0, 1.0, 0.05, 1.0, 1.0])  # key index 2 much less visible

    combined = raw_weights * gate
    gated_weights = combined / (combined.sum() + 1e-8)

    assert gated_weights[2] < raw_weights[2]
    assert torch.allclose(gated_weights.sum(), torch.tensor(1.0), atol=1e-5)
    for i in [0, 1, 3, 4]:
        assert gated_weights[i] > raw_weights[i]


def test_degenerate_all_zero_visibility_window_does_not_nan():
    tt = GatedTemporalTransformer(GatedTTConfig(window_size=5))
    with torch.no_grad():
        tt.output_proj.weight.add_(torch.randn_like(tt.output_proj.weight) * 0.02)

    num_frames = 5
    tokens, _, frame_indices = _make_inputs(batch_size=1, num_frames=num_frames)
    visibility = torch.zeros(1, num_frames)
