...
from dataset_processing.dataloading.crop_cache import compute_cropped_face  # noqa: E402
from dataset_processing.dataloading.detector_pool import get_detector  # noqa: E402
from dataset_processing.dataloading.frame_count_cache import write_shard_frame_counts  # noqa: E402
...
def main():
    parser = argparse.ArgumentParser(
        description="Populate the face-crop cache for one or more datasets before real training starts.")
    ...
    entries = load_datasets_yaml(args.datasets_yaml)
    if args.dataset != "all":
    ...
    log_path = Path(cfg.crop_cache_root) / "prewarm_logs" / f"shard_{args.shard_index}.csv"
    ...
    with open(log_path, "w", newline="") as log_file:
        ...
        for entry in entries:
            ...
            shard_frame_counts: dict[str, int] = {}
            ...
            for row in rows:
                container_path = bucket_container_path(cfg.crop_cache_root, entry.name, row.sample_id)
            ...
            for container_path, bucket_rows in tqdm(rows_by_bucket.items(), desc=entry.name):
                ...
                for row in bucket_rows:
                    ...
                    if not is_image:
                        try:
                            ...
                            continue
                        shard_frame_counts[row.sample_id] = num_frames
                    ...
                    for frame_index in frame_indices:
                        ...
                        noface_path = sentinel_path(
                            cfg.crop_cache_root, entry.name, row.sample_id, frame_index, "noface")
                        unreadable_path = sentinel_path(
                            cfg.crop_cache_root, entry.name, row.sample_id, frame_index, "unreadable")
                        ...
                        try:
                            result = compute_cropped_face(
                                load_image, get_prewarm_detector, cfg.crop_scale, cfg.image_size,
                            )
                        except Exception as exc:
                            # Defense-in-depth for anything unexpected compute_cropped_face's
                        ...
                        if not ok:
                            log_writer.writerow([
                                entry.name, row.sample_id, frame_index if frame_index is not None else "",
                                source_path, "error: failed to encode crop",
                            ])
                            continue
                        new_entries[key] = encoded.tobytes()
                ...
                if new_entries:
                    try:
                        write_bucket_entries(cfg.crop_cache_root, entry.name, bucket_rows[0].sample_id, new_entries)
                    except Exception as exc:
            ...
            if shard_frame_counts:
                write_shard_frame_counts(cfg.crop_cache_root, entry.name, args.shard_index, shard_frame_counts)
